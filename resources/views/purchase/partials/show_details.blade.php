  ...
  <div class="row invoice-info">
      ...
      <div class="col-sm-4 invoice-col">
          ...
          <address>
              ...
              @endif
              {{-- @dd($purchase) --}}
               @if (!empty($purchase->recieveLocation))
                  <br>Recieve Location: {{ $purchase->recieveLocation->name }}
              @endif
          </address>
      </div>
          ...
          <div class="table-responsive">
              <table class="table bg-gray">
                  ...
                  @php
                      ...
                      $total_before_tax_local_currency = 0.0;
                      $total_tax = 0.0;
                      $total_subtotal = 0.0;
                      $total_discount = 0.0;
                      $net_total_after_tax = 0.0;
                  @endphp
                  @foreach ($purchase->purchase_lines as $purchase_line)
                      <tr>
                          ...
                          @if ($purchase->type == 'purchase_order')
                              <td>
                                  <span class="display_currency" data-is_quantity="true"
                                      data-currency_symbol="false">{{ $purchase_line->quantity - $purchase_line->po_quantity_purchased }}</span>
                          ...
                          <td class="text-right">
                              {{-- @php
                                $unit_cost = str_replace(",", "", $purchase_line->pp_without_discount) * $currency_details->rate;
                                $int = floor($unit_cost);
                                $float = $unit_cost-$int;
                                if ($float*10 > 1)
                                    $purchase_line->pp_without_discount = ceil($unit_cost);
                                else
                                    $purchase_line->pp_without_discount = $int;
                              @endphp --}}
                              <span class="display_currency"
                                  data-currency_symbol="true" data-currency="{{ $currency_details->symbol }}">{{ $purchase_line->pp_without_discount }}</span>
                          </td>
                          <td class="text-right"><span
                                  class="display_currency">{{ $purchase_line->discount_percent }}</span> %</td>
                          <td class="no-print text-right">
                              @php
                                //$purchase_price = str_replace(",", "", $purchase_line->purchase_price) * $currency_details->rate;
                                $purchase_price = str_replace(",", "", $purchase_line->purchase_price);
                                $int = floor($purchase_price);
                                $float = $purchase_price-$int;
                                if ($float*10 > 1)
                                    $pp_without_discount = ceil($purchase_price);
                                else
                                    $pp_without_discount = $int;
                              @endphp
                              <span class="display_currency"
                                  data-currency_symbol="true" data-currency="{{ $currency_details->symbol }}">{{ $purchase_price }}</span></td>
                          <td class="no-print text-right"><span class="display_currency"
                                  data-currency_symbol="true" data-currency="{{ $currency_details->symbol }}">{{ $purchase_line->quantity * $purchase_price }}</span>
                          </td>
                          <td class="text-right"><span class="display_currency"
                                  {{-- data-currency_symbol="true" data-currency="{{ $currency_details->symbol }}">{{ str_replace(",", "", $purchase_line->item_tax) * $currency_details->rate }} </span> <br /><small> --}}
                                  data-currency_symbol="true" data-currency="{{ $currency_details->symbol }}">{{ str_replace(",", "", $purchase_line->item_tax)}} </span> <br /><small>
                  ...
                  <?php
                   // $purchase_price_inc_tax = str_replace(",","",number_format(str_replace(",", "", $purchase_line->purchase_price_inc_tax) * $currency_details->rate, 2));
                    $purchase_price_inc_tax = str_replace(",","",number_format(str_replace(",", "", $purchase_line->purchase_price_inc_tax) , 2));
                    $net_total_after_tax += $purchase_price_inc_tax * $purchase_line->quantity;
                    $total_before_tax_local_currency += $purchase_line->quantity * $purchase_line->purchase_price_inc_tax;
                  ?>
                  <td class="text-right"><span class="display_currency"
                          data-currency_symbol="true" data-currency="{{ $currency_details->symbol }}">{{ $purchase_price_inc_tax * $purchase_line->quantity }}</span></td>
                  @if ($purchase->type != 'purchase_order')
                      @php
                      ...
                      @endphp
                      <td class="text-right"><span class="display_currency"
                              data-currency_symbol="true" data-currency="{{ $currency_details->symbol }}">{{ str_replace(",", "", $sp) * $currency_details->rate }}</span></td>
                              {{-- data-currency_symbol="true" data-currency="{{ $currency_details->symbol }}">{{ str_replace(",", "", $sp) }}</span></td> --}}
                  ...
                  @endif
                  <td class="text-right"><span class="display_currency"
                          data-currency_symbol="true" data-currency="{{ $currency_details->symbol }}">{{ $purchase_price_inc_tax * $purchase_line->quantity }}</span>
                  </td>
                  </tr>
                  @php
                      $total_before_tax += $purchase_line->quantity * $purchase_price;
                  @endphp
                  @endforeach
              </table>
          </div>
                      ...
                      @forelse($purchase->payment_lines as $payment_line)
                          ...
                          <tr>
                              ...
                              <td><span class="display_currency"
                                      {{-- data-currency_symbol="true" data-currency="{{ $currency_details->symbol }}">{{  round(str_replace(",", "", $payment_line->amount) * $currency_details->rate) }}</span></td> --}}
                                      data-currency_symbol="true" data-currency="{{ $currency_details->symbol }}">{{  str_replace(",", "", $payment_line->amount)  }}</span></td>
          ...
          <td><span class="display_currency pull-right">{{ $total_before_tax }}</span></td>
                  ...
                  <tr>
                      ...
                      <td><span class="display_currency pull-right"
                              data-currency_symbol="true" data-currency="{{ $currency_details->symbol }}">{{ $net_total_after_tax }}</span></td>
                  </tr>
                  <tr>
                      ...
                      <td>
                          <span class="display_currency pull-right" data-currency_symbol="true" data-currency="{{ $currency_details->symbol }}">
                              @if ($purchase->discount_type == 'percentage')
                                  {{ ($purchase->discount_amount * $total_before_tax) / 100 }}
                              @else
                                  {{ $purchase->discount_amount }}
                                  {{-- {{ $purchase->discount_amount * $currency_details->rate }} --}}
                              @endif
                          </span>
                      </td>
                  </tr>
                          ...
                          @if (!empty($purchase_taxes))
                              @foreach ($purchase_taxes as $k => $v)
                                  <strong><small>{{ $k }}</small></strong> = <span
                                      ...
                                      {{-- data-currency_symbol="true" data-currency="{{ $currency_details->symbol }}">{{ $v * $currency_details->rate }}</span><br> --}}
                                      data-currency_symbol="true" data-currency="{{ $currency_details->symbol }}">{{ $v  }}</span><br>
                              @endforeach
                          @else
                              0.00
                          @endif
                  ...
                  @if (!empty($purchase->shipping_charges))
                      <tr>
                          ...
                          <td><b>(+)</b></td>
                          {{-- <td><span class="display_currency pull-right">{{ $purchase->shipping_charges * $currency_details->rate }}</span></td> --}}
                  ...
                  @if (!empty($purchase->additional_expense_value_1) && !empty($purchase->additional_expense_key_1))
                      <tr>
                          ...
                          <td><b>(+)</b></td>
                          <td><span
                                  {{-- class="display_currency pull-right">{{ $purchase->additional_expense_value_1 * $currency_details->rate }}</span> --}}
                  ...
                  @if (!empty($purchase->additional_expense_value_2) && !empty($purchase->additional_expense_key_2))
                      <tr>
                          ...
                          <td><b>(+)</b></td>
                          <td><span
                                  {{-- class="display_currency pull-right">{{ $purchase->additional_expense_value_2 * $currency_details->rate }}</span> --}}
                  ...
                  @if (!empty($purchase->additional_expense_value_3) && !empty($purchase->additional_expense_key_3))
                      <tr>
                          ...
                          <td><b>(+)</b></td>
                          <td><span
                                  {{-- class="display_currency pull-right">{{ $purchase->additional_expense_value_3 * $currency_details->rate }}</span> --}}
                  ...
                  @if (!empty($purchase->additional_expense_value_4) && !empty($purchase->additional_expense_key_4))
                      <tr>
                          ...
                          <td><b>(+)</b></td>
                          <td><span
                                  {{-- class="display_currency pull-right">{{ $purchase->additional_expense_value_4 * $currency_details->rate }}</span> --}}
                  ...
                  <tr>
                      ...
                      <td></td>
                      <td><span class="display_currency pull-right"
                              {{-- data-currency_symbol="true" data-currency="{{ $currency_details->symbol }}">{{ $purchase->final_total* $currency_details->rate }}</span></td> --}}
                              data-currency_symbol="true" data-currency="{{ $currency_details->symbol }}">{{ $purchase->final_total }}</span></td>
                  </tr>
              </table>
          </div>
      </div>
