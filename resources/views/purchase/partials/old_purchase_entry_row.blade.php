@foreach( $variations as $variation)
    <tr @if(!empty($purchase_order_line)) data-purchase_order_id="{{$purchase_order_line->transaction_id}}" @endif @if(!empty($purchase_requisition_line)) data-purchase_requisition_id="{{$purchase_requisition_line->transaction_id}}" @endif>
        ...
        <td>
            ...
            <input type="hidden" class="base_unit_cost" value="{{ $variation->default_purchase_price * $currency_exchange_rate->rate }}">
            <input type="hidden" class="base_unit_selling_price" value="{{ $variation->sell_price_inc_tax * $currency_exchange_rate->rate }}">
        ...
        <td>
            ...
            @endphp
            {!! Form::text('purchases[' . $row_count . '][pp_without_discount]', $pp_without_discount * $currency_exchange_rate->rate, ['class' => 'form-control input-sm purchase_unit_cost_without_discount input_number', 'required']); !!}

            @if(!empty($last_purchase_line))
                <br>
                <small class="text-muted">@lang('lang_v1.prev_unit_price'): @format_currency($last_purchase_line->pp_without_discount * $currency_exchange_rate->rate, $currency_exchange_rate->code)</small>
            @endif
        </td>
