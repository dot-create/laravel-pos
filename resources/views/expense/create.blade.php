			...
			<div class="row">
				...
				<div class="col-sm-4">
					<div class="form-group">
						...
						{!! Form::select('expense_type', ['fixed' => 'Fixed Amount', 'percentage' => 'Percentage'], null, ['class' => 'form-control', 'id' => 'expense_type']) !!}
					</div>
				</div>

				<div class="col-sm-4">
					<div class="form-group">
						{!! Form::label('expense_value', 'Expense Value:*') !!}
						{!! Form::text('expense_value', null, ['class' => 'form-control input_number', 'id' => 'expense_value', 'placeholder' => 'Amount or %']) !!}
					</div>
				</div>

				<div class="col-sm-4">
					<div class="form-group">
						{!! Form::label('calculated_expense_amount', 'Calculated Expense Amount:') !!}
						{!! Form::text('calculated_expense_amount', null, ['class' => 'form-control input_number', 'readonly' => true, 'id' => 'calculated_expense_amount']) !!}
					</div>
				</div>
...
$('#search_purchase').on('keyup', function() {
	...
	function calculateExpenseAmount() {
		var base_amount = __read_number($('#final_total'));
		var expense_type = $('#expense_type').val();
		var expense_value = parseFloat($('#expense_value').val());

		if (expense_type === 'percentage' && !isNaN(expense_value)) {
			var calc_amount = (base_amount * expense_value) / 100;
			$('#calculated_expense_amount').val(__number_f(calc_amount, false));
		} else if (expense_type === 'fixed' && !isNaN(expense_value)) {
			$('#calculated_expense_amount').val(__number_f(expense_value, false));
		} else {
			$('#calculated_expense_amount').val('');
		}
		calculateExpensePaymentDue();
	}

	$('#expense_type, #expense_value, #final_total').on('change keyup', function() {
