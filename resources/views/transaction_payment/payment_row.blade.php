    ...
    @endphp
        ...
        <div class="modal-body">
            <div class="row">
                ...
                <div class="col-md-4">
                    <div class="well">
                        <strong>@lang('sale.total_amount'): </strong><span class="display_currency payable_total_amount"
                            {{-- data-currency_symbol="true" data-currency="{{ $currency_details->symbol }}">{{ $expense ? str_replace(",","", $transaction->final_total) / $currency_details->rate : str_replace(",","", $transaction->final_total) * $currency_details->rate }}</span><br> --}}
                            data-currency_symbol="true" data-currency="{{ $currency_details->symbol }}">{{ $expense ? str_replace(",","", $transaction->final_total)  : round(str_replace(",","", $transaction->final_total))  }}</span><br>
                ...
                <div class="col-md-12">
                    @if (!empty($transaction->contact))
                        <strong>@lang('lang_v1.advance_balance'):</strong> <span class="display_currency"
                            data-currency_symbol="true" data-currency="{{ $currency_details->symbol }}">{{ $expense ? str_replace(',', '', $transaction->contact->balance) / $currency_details->rate : str_replace(',', '', $transaction->contact->balance) * $currency_details->rate }}</span>

                        {!! Form::hidden('advance_balance', $expense ? str_replace(',', '', $transaction->contact->balance) / $currency_details->rate : str_replace(',', '', $transaction->contact->balance) * $currency_details->rate , [
                            'id' => 'advance_balance',
                ...
                <div class="col-md-4">
                    <div class="form-group">
                        ...
                        <div class="input-group">
                            ...
                            {!! Form::text('amount', @num_format( $expense ? $payment_line->amount : $payment_line->amount ), [
                                'class' => 'form-control input_number payment_amount edit_payment_amount',
                                'required',
                                'placeholder' => 'Amount',
                                'data-rule-max-value' => $expense ? number_format($payment_line->amount, 2) : number_format($payment_line->amount, 2),
