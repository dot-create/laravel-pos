@php
	...
	//$currency_details->rate = isset($currency_details) ? $currency_details->rate : "1";
	//$currency_details->rate =  "1";
@endphp
...
<tr class="product_row" data-row_index="{{$row_count}}" @if(!empty($so_line)) data-so_id="{{$so_line->transaction_id}}" @endif>
	...
	<td>
		...
		<input type="text" data-min="1" 
			...
			@if($product->enable_stock && empty($pos_settings['allow_overselling']) && $is_sales_order==true )
			...
			@endif 
		>
		{{-- @dd($product->enable_stock,$is_sales_order); --}}
	...
	@if(!empty($is_direct_sell))
		...
		@php
			...
			if($action !== "edit"){
			    $pos_unit_price= number_format($pos_unit_price* $currency_details->rate ,2);

			}
		@endphp
		<td class="@if(!auth()->user()->can('edit_product_price_from_sale_screen')) hide @endif">
			{{-- <input type="text" name="products[{{$row_count}}][unit_price]" class="form-control pos_unit_price input_number mousetrap" value="{{@num_format((float) $pos_unit_price* (float)$currency_details->rate)}}" @if(!empty($pos_settings['enable_msp'])) data-rule-min-value="{{$pos_unit_price}}" data-msg-min-value="{{__('lang_v1.minimum_selling_price_error_msg', ['price' => @num_format($pos_unit_price)])}}" @endif>  --}}
			<input type="text" name="products[{{$row_count}}][unit_price]" class="form-control pos_unit_price input_number mousetrap" value="{{$pos_unit_price}}" @if(!empty($pos_settings['enable_msp'])) data-rule-min-value="{{$pos_unit_price}}" data-msg-min-value="{{__('lang_v1.minimum_selling_price_error_msg', ['price' => @num_format($pos_unit_price)])}}" @endif> 

			@if(!empty($last_sell_line))
				<br>
				<small class="text-muted">@lang('lang_v1.prev_unit_price'): @format_currency((float) $last_sell_line->unit_price_before_discount * (float)$currency_details->rate)</small>
				{{-- <small class="text-muted">@lang('lang_v1.prev_unit_price'): @format_currency((float) $last_sell_line->unit_price_before_discount)</small> --}}
			@endif
		</td>
		<td @if(!$edit_discount) class="hide" @endif>
			{!! Form::text("products[$row_count][line_discount_amount]", @num_format((float) $discount_amount), ['class' => 'form-control input_number row_discount_amount']); !!}<br>
			...
			@if(!empty($last_sell_line))
				...
				<small class="text-muted">
					...
					@else
						@format_currency((float)$last_sell_line->line_discount_amount * (float)$currency_details->rate)
						{{-- @format_currency((float)$last_sell_line->line_discount_amount ) --}}
					@endif
				</small>
			@endif
		</td>
		<td class="text-center {{$hide_tax}}">
			{{-- {!! Form::hidden("products[$row_count][item_tax]", @num_format((float)$item_tax * (float)$currency_details->rate), ['class' => 'item_tax']); !!}) --}}
			{!! Form::hidden("products[$row_count][item_tax]", @num_format((float)$item_tax ), ['class' => 'item_tax']); !!})
	...
	@endif
	<td class="{{$hide_tax}}">
    {{-- <input type="text" name="products[{{$row_count}}][unit_price_inc_tax]" class="form-control pos_unit_price_inc_tax input_number" value="{{@num_format((float)$unit_price_inc_tax * (float)$currency_details->rate)}}" @if(!$edit_price) readonly @endif @if(!empty($pos_settings['enable_msp'])) data-rule-min-value="{{$unit_price_inc_tax}}" data-msg-min-value="{{__('lang_v1.minimum_selling_price_error_msg', ['price' => @num_format($unit_price_inc_tax)])}}" @endif> --}}
    <input type="text" name="products[{{$row_count}}][unit_price_inc_tax]" class="form-control pos_unit_price_inc_tax input_number" value="{{@num_format((float)$unit_price_inc_tax) }}" @if(!$edit_price) readonly @endif @if(!empty($pos_settings['enable_msp'])) data-rule-min-value="{{$unit_price_inc_tax}}" data-msg-min-value="{{__('lang_v1.minimum_selling_price_error_msg', ['price' => @num_format($unit_price_inc_tax)])}}" @endif>
</td>
...
<td class="text-center">
    ...
    @endphp
	@if($action!=='edit')
		<input type="{{$subtotal_type}}" class="form-control pos_line_total @if(!empty($pos_settings['is_pos_subtotal_editable'])) input_number @endif" value="{{@num_format($product->quantity_ordered*$unit_price_inc_tax * (float)$currency_details->rate )}}">
		<span class="display_currency pos_line_total_text @if(!empty($pos_settings['is_pos_subtotal_editable'])) hide @endif" data-currency="{{ $currency_details->symbol }}" data-currency_symbol="true">{{@num_format($product->quantity_ordered*$unit_price_inc_tax * (float)$currency_details->rate)}}</span>
	@else 
     ...
     <span class="display_currency pos_line_total_text @if(!empty($pos_settings['is_pos_subtotal_editable'])) hide @endif" data-currency="{{ $currency_details->symbol }}" data-currency_symbol="true">{{$product->quantity_ordered*$unit_price_inc_tax }}</span> 
	@endif
