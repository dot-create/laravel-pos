          ...
          <table class="table bg-gray">
            ...
            @foreach($sell->payment_lines as $payment_line)
              ...
              <tr>
                ...
                {{-- <td><span class="display_currency" data-currency_symbol="true" data-currency="{{ $currency_details->symbol }}">{{ round($payment_line->amount*$currency_details->rate) }}</span></td> --}}
                <td><span class="display_currency" data-currency_symbol="true" data-currency="{{ $currency_details->symbol }}">{{ round($payment_line->amount) }}</span></td>
                <td>
        ...
        <div class="table-responsive">
          <table class="table bg-gray">
            <tr>
              <th>{{ __('sale.total') }}: </th>
              <td></td>
              <td><span class="display_currency pull-right" data-currency_symbol="true" data-currency="{{ $currency_details->symbol }}">{{ $sell->total_before_tax }}</span></td>
            </tr>
            <tr>
              <th>{{ __('sale.discount') }}:</th>
              <td><b>(-)</b></td>
              <td><div class="pull-right"><span class="display_currency" @if( $sell->discount_type == 'fixed') data-currency_symbol="true"  data-currency="{{ $currency_details->symbol }}" @endif>{{ $sell->discount_type == 'fixed' ? $sell->discount_amount : $sell->discount_amount }}</span> @if( $sell->discount_type == 'percentage') {{ '%'}} @endif</span></div></td>
            </tr>
            @if(in_array('types_of_service' ,$enabled_modules) && !empty($sell->packing_charge)) 
              <tr>
                <th>{{ __('lang_v1.packing_charge') }}:</th>
                <td><b>(+)</b></td>
                <td><div class="pull-right"><span class="display_currency" @if( $sell->packing_charge_type == 'fixed') data-currency_symbol="true"  data-currency="{{ $currency_details->symbol }}" @endif>{{ $sell->packing_charge_type == 'fixed' ? $sell->packing_charge : $sell->packing_charge }}</span> @if( $sell->packing_charge_type == 'percent') {{ '%'}} @endif </div></td>
              </tr>
            @endif
            @if(session('business.enable_rp') == 1 && !empty($sell->rp_redeemed) )
              <tr>
                ...
                <td><b>(-)</b></td>
                <td> <span class="display_currency pull-right" data-currency_symbol="true"  data-currency="{{ $currency_details->symbol }}">{{ $sell->rp_redeemed_amount }}</span></td>
              </tr>
            @endif
            <tr>
              ...
              <td class="text-right">
                @if(!empty($order_taxes))
                  @foreach($order_taxes as $k => $v)
                    <strong><small>{{$k}}</small></strong> - <span class="display_currency pull-right" data-currency_symbol="true" data-currency="{{ $currency_details->symbol  }}">{{ $v }}</span><br>
                  @endforeach
                @else
                0.00
                @endif
            ...
            <tr>
              ...
              <td><b>(+)</b></td>
              <td><span class="display_currency pull-right" data-currency_symbol="true"  data-currency="{{ $currency_details->symbol }}">{{ $sell->shipping_charges }}</span></td>
            </tr>
            ...
            <tr>
              ...
              <td></td>
              <td><span class="display_currency pull-right" data-currency_symbol="true" data-currency="{{ $currency_details->symbol }}">{{ $sell->round_off_amount }}</span></td>
            </tr>
            <tr>
              <th>{{ __('sale.total_payable') }}: </th>
              <td></td>
              <td><span class="display_currency pull-right" data-currency_symbol="true" data-currency="{{ $currency_details->symbol }}">{{ $sell->final_total }}</span></td>
            </tr>
            @if($sell->type != 'sales_order')
            <tr>
              <th>{{ __('sale.total_paid') }}:</th>
              <td></td>
              <td><span class="display_currency pull-right" data-currency_symbol="true"  data-currency="{{ $currency_details->symbol }}">{{ $total_paid }}</span></td>
            </tr>
            <tr>
              ...
              <td>
                ...
                @endphp
                <span class="display_currency pull-right" data-currency_symbol="true" data-currency="{{ $currency_details->symbol }}">{{ ($sell->final_total - $total_paid) }}</span></td>
            </tr>
            @endif
          </table>
        </div>
