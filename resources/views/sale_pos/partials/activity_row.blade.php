@php
    //$exchange_rate = isset($currency_details) ? $currency_details->rate : 1;
    $exchange_rate = isset($currency_details) ? $currency_details->p_exchange_rate : 1;
...
@if(!empty($final_total) && $final_total != $old_final_total)
    ...
    <td class="width-50 text-left">
        @if(!empty($old_final_total))
            <span class="label bg-info">{{$currency_details->symbol}} {{ str_replace(",","", $old_final_total) * $exchange_rate }}</span> --> 
        @endif
         <span class="label bg-info">{{$currency_details->symbol}} {{ str_replace(",","", $total_paid) * $exchange_rate }}</span>
     </td>
    </tr>
@endif
