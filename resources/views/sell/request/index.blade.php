    ...
    <h1>
        @switch($status)
            @case('listPendingRequests')
                @lang('request.pending_request_items')
            @break
            @case('readyTodraft')
                @lang('request.ready_draft_request')
            @break
            @case('draftRequest')
                @lang('request.draft_request')
            @break

            @case('listQuoteRequest')
                @lang('request.list_quote_request')
            @break
            @case('acceptedQuoteRequest')
                @lang('request.accepted_quote_request')
            @break
            @case('rejectedQuoteRequest')
                @lang('request.rejected_quote_request')
            @break
            @case('disputeQuoteRequest')
                @lang('request.dispute_quote_request')
            @break
            @case('pendingPurchaseRequest')
                @lang('request.pending_purchase_request')
            @break
            @case('purchaseOrderListRequest')
                @lang('request.purchase_order_list_request')
            @break
            @default
...
<section class="content no-print">
    @component('components.filters', ['title' => __('report.filters')])
        ...
        <div class="col-md-3">
            <div class="form-group">
                {!! Form::label('request_list_filter_status',  __('request.request_status') . ':') !!}
                {!! Form::select('request_list_filter_status', $orderStatuses, null, ['class' => 'form-control select2', 'style' => 'width:100%', 'placeholder' => __('lang_v1.all')]); !!}
            </div>
        </div>
        <div class="col-md-3">
            <div class="form-group">
