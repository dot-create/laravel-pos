...
<input type="hidden" id="__p_currency_symbol" value="HNK">
<input type="hidden" id="__p_currency_thousand_separator" value=",">
<input type="hidden" id="__p_currency_decimal_separator" value=".">
...
<div class="row">
	<div class="col-sm-3">
		<div class="form-group">
			...
			'id' => 'select_location_id', 'onchange' => 'checkLocations()',
...
<input type="hidden" id="item_addition_method" value="{{$business_details->item_addition_method}}">
		        ...
		        @if((!empty($pos_settings['enable_sales_order']) && $sale_type != 'sales_order') || $is_order_request_enabled)
					...
					<div class="col-md-4">
						<div class="form-group">
							...
							{!! Form::select('recieve_location_id', $business_locations, $default_location,
							 ['class' => 'form-control select2', 'placeholder' => __('messages.please_select'), 'required','id' => 'recieve_location_id','onchange' => 'checkLocations()'], $bl_attributes); !!}
						</div>
					</div>	
					<div class="clearfix"></div>
				@endif
	...
	<div class="row">
		<p id="form_error" class="text-danger text-center"></p>
...
@section('javascript')
	...
	function checkLocations() {
		var recieve_location_id=$('#recieve_location_id').val();
		var main_location=$('#location_id').val();
		if (recieve_location_id == main_location) {
			$('#form_error').text('Business location and Foriegn locaiton should be different');
			$('#submit-sell').attr('disabled', true);
			$('#save-and-print').attr('disabled', true);
		} else {
			$('#form_error').text('');
			$('#submit-sell').attr('disabled', false);
			$('#save-and-print').attr('disabled', false);
		}
	}
    	$(document).ready( function() {
			$('#recieve_location_id').change(function(){
				var main_location=$('#location_id').val();
    			if ($(this).val() == main_location) {
    				$('#form_error').text('Business location and Foriegn locaiton should be different');
    				$('#submit-sell').attr('disabled', true);
    				$('#save-and-print').attr('disabled', true);
    			} else {
		    ...
		    $(document).on('change', '#prefer_payment_method', function(e) {
			 ...
			 $(document).on('change', '#select_location_id', function(e) {
