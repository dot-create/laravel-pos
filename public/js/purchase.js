$(document).ready(function() {
    ...
    let getCustomerOrsupplierUrl='/purchases/get_suppliers';

    if(requestPage){
        getCustomerOrsupplierUrl='/purchases/get_customers'
    }
    //get suppliers
    $('#supplier_id').select2({
        ajax: {
            url: getCustomerOrsupplierUrl,
            dataType: 'json',
            delay: 250,
            ...
            .autocomplete({
                source: function(request, response) {
                    $.getJSON(
                        ...
                        { location_id: $('#location_id').val(), term: request.term,check_enable_stock: isCheckStck},
                        response
                    );
                },
                minLength: 2,
                ...
                select: function(event, ui) {
                    $(this).val(null);
                    get_purchase_entry_row(ui.item.product_id, ui.item.variation_id,requestPage);
                },
            })
    ...
    request_table = $('#request_table').DataTable({
        ...
        ajax: {
            ...
            data: function(d) {
                ...
                console.log('statuss',$('#request_list_filter_status').length);
                if ($('#request_list_filter_status').length) {
    ...
    pending_request_items_table = $('#pending_request_items_table').DataTable({
        processing: true,
        serverSide: true,
        scrollY: "75vh",
        scrollX:        true,
        ...
        columns: [
            ...
            { data: 'product_type', name: 'type' }, 
            { data: 'product_sku', name: 'product_sku' },
            { data: 'availability_status', name: 'status' },
            { data: 'action', name: 'action' },
        ],
        fnDrawCallback: function(oSettings) {
            __currency_convert_recursively($('#pending_request_items_table'));
        },
        createdRow: function(row, data, dataIndex) {
            $(row)
                .find('td:eq(5)')
    ...
    $(document).on(
        ...
        function() {
            pending_request_items_table.ajax.reload();
        }
    );
    readyDraft_request_items_table = $('#readyDraft_request_items_table').DataTable({
        processing: true,
        serverSide: true,
        scrollY: "75vh",
        scrollX: true,
        ...
        ajax: {
            ...
            data: function(d) {
                ...
                d.start_date = start;
                d.end_date = end;
                __datatable_ajax_callback(d);
            },
            error: function(xhr, error, thrown) {
                console.error('DataTable AJAX error:', xhr.status, xhr.responseText);
                // Optional: Show error to user
                alert('Server error: ' + xhr.status + '. Trying again in 5 seconds...');
                
                // Retry after delay
                // setTimeout(function() {
                //     readyDraft_request_items_table.ajax.reload();
                // }, 5000);
            }
        },
        aaSorting: [[1, 'desc']],
        ...
        fnDrawCallback: function(oSettings) {
            __currency_convert_recursively($('#readyDraft_request_items_table'));
        },
        createdRow: function(row, data, dataIndex) {
            $(row)
                .find('td:eq(5)')
    ...
    quote_request_items_table = $('#quote_request_items_table').DataTable({
        processing: true,
        serverSide: true,
        scrollY: "75vh",
        scrollX:        true,
    ...
    $(document).on(
        'change',
        '#quote_request_items_table,\
        #request_list_filter_status,\
        #request_list_filter_sku_model',
        function() {
            quote_request_items_table.ajax.reload();
        }
    );
    quote_reject_table = $('#quote_reject_table').DataTable({
        processing: true,
        serverSide: true,
        scrollY: "75vh",
        scrollX:        true,
    ...
    draft_request_items_table = $('#draft_request_items_table').DataTable({
        processing: true,
        serverSide: true,
        scrollY: "75vh",
        scrollX:        true,
...
function get_purchase_entry_row(product_id, variation_id,requestPage) {
    if (product_id) {
...
function append_purchase_lines(data, row_count, trigger_change = false,requestPage=false) {
    $(data)
        .find('.purchase_quantity')
        .each(function() {
            row = $(this).closest('tr');
            if(requestPage){
                $('#request_entry_table tbody').append(
...
function update_table_total() {
    ...
    var currency = $('#p_symbol').val();
    if($('#request_entry_table tbody').length >0){
        $('#request_entry_table tbody')
        .find('tr')
        .each(function() {
            total_quantity += __read_number($(this).find('.purchase_quantity'), true);
            total_currectRowQuantity = __read_number($(this).find('.purchase_quantity'), true);
            
            let total_Avaliable_quantity = $(this).find('.avaliableQty');
            if(total_Avaliable_quantity != undefined){
                total_Avaliable_quantity= parseInt(total_Avaliable_quantity.text());
                let selected =$(this).find('#avaliabilityStatus');
                if(total_Avaliable_quantity >= total_currectRowQuantity && selected !=undefined){
                    $(selected).val('Avaliable').change();
                }
                else if(total_Avaliable_quantity < total_currectRowQuantity && total_Avaliable_quantity > 0 && selected !=undefined){
                    $(selected).val('Partial Avalaible').change();
                }
                else if(total_Avaliable_quantity <=  0 && selected !=undefined){
                    $(selected).val('Not Avalaible').change();
                }
                else{
                    $(selected).val('Not Avalaible').change();
                }
                console.log(total_Avaliable_quantity,'total_Avaliable_quantity');
            }
            if($(this).find('.row_subtotal_before_tax_').length > 0){
                let qty=__read_number($(this).find('.purchase_quantity'), true);
                let value=__read_number(
                    $(this).find('.row_subtotal_before_tax_'),
                    true
                );
                total_st_before_tax+=qty * value;
                newItem=qty * value;
            ...
            // total_subtotal += __read_number($(this).find('.row_subtotal_before_tax_'), true);
            total_subtotal +=newItem;
        });
    ...
    // $('#total_subtotal').text(__currency_trans_from_en(total_quantity * total_subtotal, true, true, "", false, currency));
    $('#total_subtotal').text(__currency_trans_from_en(total_subtotal, true, true, "", false, currency));
    console.log('total_subtotal',total_subtotal);
...
function update_grand_total() {
    ...
    $('#discount_calculated_amount').text(__currency_trans_from_en(discount, true, true, "", false, currency));
    ...
    $('#tax_calculated_amount').text(__currency_trans_from_en(tax, true, true, "", false, currency));
    ...
    $('#grand_total').text(__currency_trans_from_en(grand_total, true, true, "", false, currency));

    $('#payment_due').text(__currency_trans_from_en(due, true, true, "", false, currency));
...
$(document).on('change', 'input.payment-amount', function() {
    ...
    // var symbol =  $('#__symbol').val();
    var symbol = $('#p_symbol').val();
    $('#payment_due').text(__currency_trans_from_en(bal, true, true, "", false, symbol));
});
...
$(document).on('click', 'button#submit_purchase_form', function(e) {
    ...
    e.preventDefault();
    if(requestPage){
        if ($('table#request_entry_table tbody tr').length <= 0) {
...
function update_currency_details() {
    ...
    if (location_id) {
        $.ajax({
            // url: '/locations/' + location_id + '/get_currency_details',
            url: '/get-currency/' + location_id,
            dataType: 'json',
            success: function(response) {
                ...
                $("#advance_balance_text").text(response.currency.symbol + " 0.00")
                var html='';
