...
function __currency_trans_from_en(
    ...
    if(changeable_currency) {
        symbol = changeable_currency;
    }
    return accounting.formatMoney(input, symbol, 4, thousand, decimal, format);
}
...
function __currency_convert_recursively(element, use_page_currency = false) {
    element.find('.display_currency').each(function() {
        ...
        var changeable_currency = $(this).data('currency');

        $(this).text(__currency_trans_from_en(value, show_symbol, use_page_currency, __currency_precision, is_quantity, changeable_currency));
    });
}
