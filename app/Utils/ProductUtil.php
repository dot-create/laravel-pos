...
class ProductUtil extends Util
{
    ...
    {
        // dd($variation_id, $business_id, $location_id);
    ...
    /**
     * Add rack details using storage locations
     *
     * @param int $business_id
     * @param int $product_id
     * @param array $product_racks [location_id => [rack, row, position]]
     * @return void
     */
    public function addRackDetails($business_id, $product_id, $product_racks)
    {
        if (!empty($product_racks)) {
            ...
            foreach ($product_racks as $location_id => $details) {
                ...
                $storage_location = StorageLocation::firstOrCreate(
                    [
                        'business_id' => $business_id,
                        'location_id' => $location_id,
                        'rack' => $details['rack'] ?? null,
                        'row' => $details['row'] ?? null,
                        'position' => $details['position'] ?? null
                    ]
                );

                // Create product rack assignment
                ProductRack::create([
                    'business_id' => $business_id,
                    'location_id' => $location_id,
                    'product_id' => $product_id,
    ...
    {
        if (!empty($product_racks)) {
            // Delete existing rack assignments for these locations
            $location_ids = array_keys($product_racks);
            
            ProductRack::where('business_id', $business_id)
                ->where('product_id', $product_id)
                ->whereHas('storageLocation', function ($q) use ($location_ids) {
        ...
        foreach ($input_data as $data) {
            ...
            if (isset($data['purchase_line_id'])) {
                ...
                if ($transaction->status == 'received') {
                    // $this->updateProductQuantity($transaction->location_id, $data['product_id'], $data['variation_id'], $new_quantity_f, 0, $currency_details); CBy Bilal
                    $this->updateProductQuantity($transaction->recieve_location_id, $data['product_id'], $data['variation_id'], $new_quantity_f, 0, $currency_details);
                }
            }

            $purchase_line->quantity = $new_quantity;
            $purchase_line->pp_without_discount = ($this->num_uf($data['pp_without_discount'], $currency_details)) / $multiplier;
            $purchase_line->discount_percent = $this->num_uf($data['discount_percent'], $currency_details);
            $purchase_line->purchase_price = ($this->num_uf($data['purchase_price'], $currency_details)) / $multiplier;
            $purchase_line->purchase_price_inc_tax = ($this->num_uf($data['purchase_price_inc_tax'], $currency_details)) / $multiplier;
            $purchase_line->item_tax = ($this->num_uf($data['item_tax'], $currency_details)) / $multiplier;
            ...
            if ($enable_product_editing == 1 && $transaction->type == 'purchase') {
                $multiplier=$currency_details->rate;
                ...
                $variation_data['pp_without_discount'] = ($this->num_uf($data['pp_without_discount'], $currency_details)) / $multiplier;
    ...
    public function getProductStockDetails($business_id, $filters, $for)
    {
        ...
        // if (isset($filters['stock_type']) && $filters['stock_type']=='non_stock'){
        //     $query->where('stock_type', 'non_stock');
        // }else{
        //     $query->whereNotIn('stock_type',['non_stock']);
        //     $query->orWhereNull('stock_type');
        // }
        ...
        if (isset($filters['stock_type']) && $filters['stock_type'] == 'non_stock') {
            ...
            $query->where(function ($q) {
                $q->whereNotIn('stock_type', ['non_stock'])
                ->orWhereNull('stock_type');
            });
        }
    ...
    {
        // Added BY Bilal
        $purchaseTransactions = Transaction::leftjoin('transaction_sell_lines as sl', 
            ...
            ->where('transactions.recieve_location_id', $location_id)
        ...
        $otherTransactions = Transaction::leftjoin('transaction_sell_lines as sl', 
            'sl.transaction_id', '=', 'transactions.id')
            ->leftjoin('purchase_lines as pl', 
                'pl.transaction_id', '=', 'transactions.id')
            ->leftjoin('stock_adjustment_lines as al', 
            ...
            ->whereIn('transactions.type', ['sell', 'stock_adjustment', 'opening_stock', 'sell_transfer', 'purchase_transfer', 'production_purchase', 'purchase_return', 'sell_return', 'production_sell'])
            ->select(
                'transactions.id as transaction_id',
                'transactions.type as transaction_type',
                'sl.quantity as sell_line_quantity',
        ...
        $stock_history = $purchaseTransactions->concat($otherTransactions);
        $stock_history_array = [];
        $stock = 0;
    ...
    public function getAvaliableQty($business_id,$id,$location_id){
        $stock_details = $this->getVariationStockDetails($business_id, $id, $location_id);
            // dd($stock_details);
            $stock_history = $this->getVariationStockHistory($business_id, $id, $location_id);
            ...
            $purchaseslines = PurchaseLine::with('transaction')
                ->where('variation_id', $id)
                ->whereHas('transaction', function ($query) use ($business_id,$location_id) {
                    $query->where(function ($q) {
                        $q->where('type', 'purchase')
                        ->whereIn('status', ['ordered', 'pending']);
                    })->orWhere(function ($q) {
                        $q->where('type', 'purchase_requisition')
                        ->where('status', 'partial');
                    })->orWhere(function ($q) {
                        $q->where('type', 'purchase_order')
                        ->whereIn('status', ['ordered', 'pending']);
                    })->where('location_id', $location_id)
            ...
            foreach ($purchaseslines as $purchasesline) {
                $type = $purchasesline->transaction->type;

                if ($type === 'purchase_order') {
                    $intransitQuantity += (float)$purchasesline->quantity - (float)$purchasesline->po_quantity_purchased;
                } else {
                    $intransitQuantity += (float)$purchasesline->quantity;
                }
            }
            $orderPurchase=0;
            $purchaseslines = PurchaseLine::where([
                'variation_id'=>$id,

            ])
            ->whereHas('transaction', function ($query) {
                $query->where('type', 'purchase_requisition')
                      ->where('status', 'ordered');
            })
            ->get();
            
            foreach($purchaseslines as $purchasesline){
                $orderPurchase+=(float)$purchasesline->quantity;
            }
            $totalCommitedQty = CustomerRequest::where(['status'=> 'AcceptedQuote','business_location_id'=>request()->input('location_id')])
            ...
            $totalRequestQty=CustomerRequest::where(['business_location_id'=>request()->input('location_id')])
                            ->whereHas('items', function ($query) use ($id) {
                                $query->where('variation_id', $id);
                            })
                            ->withSum(['items' => function ($query) use ($id) {
            ...
            $stockOnHand=max(0,$stock_details['current_stock']);    
            $avaliabilityQty = $stockOnHand + $intransitQuantity - $totalCommitedQty;
            $iprQty=$orderPurchase;
            $avaliabilityQty = $avaliabilityQty + $iprQty;
            // dd($avaliabilityQty);
            return ['avaliabilityQty'=>$avaliabilityQty,'intransitQuantity'=>$intransitQuantity,'stockOnHand'=>$stockOnHand];
    }
}
