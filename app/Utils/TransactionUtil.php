...
class TransactionUtil extends Util
{
    ...
    public function createSellTransaction($business_id, $input, $invoice_total, $user_id, $uf_data = true)
    {
        try {
            ...
            $transaction = Transaction::create([
                ...
                'recieve_location_id' => isset( $input['recieve_location_id'] ) ? $input['recieve_location_id'] : null,
                'type' => $sale_type,
                'status' => $input['status'],
            ...
            throw $e; // Optional: rethrow to break the transaction if needed
        }
    }
    ...
    public function getReceiptDetails($transaction_id, $location_id, $invoice_layout, $business_details, $location_details, $receipt_printer_type)
    {
        ...
        $transaction = Transaction::with('recieveLocation')->find($transaction_id);
        if($transaction->recieveLocation){
            $foreign_currency = Currency::where('id', $transaction->recieveLocation->currency_id)->first();
        }
        ...
        $output = [
            ...
            'foreign_location' => ($transaction->recieveLocation)?$transaction->recieveLocation->name:'',
        ...
        if (in_array($transaction_type, ['sell', 'sales_order'])) {
            ...
            $output['foreign_subtotal_exc_tax'] =$subtotal_exc_tax;
            $output['foreign_total_line_discount'] = !empty($total_line_discount) ? $total_line_discount: 0;
        ...
        $output['foreign_discount'] = ($discount != 0) ? $discount : 0;
        ...
        $output['foreign_tax'] = ($transaction->tax_amount != 0) ?$transaction->tax_amount: 0;
        ...
        $output['foreign_shipping_charges'] = ($transaction->shipping_charges != 0) ? $transaction->shipping_charges : 0;
        ...
        $output['foreign_packing_charge'] = ($transaction->packing_charge != 0) ? $transaction->packing_charge : 0;
        ...
        $output['foreign_total'] = $transaction->final_total;
        ...
        if (!empty($il->common_settings['show_total_in_words'])) {
            ...
            $totalForWord = str_replace(",", "", number_format($transaction->final_total, 2));
            $output['total_in_words'] = $this->numToWord($totalForWord , null, $word_format);
        }
    ...
    public function getRequestReceiptDetails($request, $location_id, $invoice_layout, $business_details, $location_details, $receipt_printer_type)
    {
        $il = $invoice_layout;
        $foreign_currency=null;
        $foreign_location=BusinessLocation::where('id',$request->foreign_business_location_id)->first();
        $foreign_currency = Currency::where('id', $foreign_location->currency_id)->first();
        ...
        $output = [
            ...
            'table_unit_price_label' => $il->table_unit_price_label,
            'table_subtotal_label' => $il->table_subtotal_label,
            'foreign_location' => $foreign_location->name,
            'foreign_currency' => $foreign_currency,
        ...
        //Invoice info
        $output['invoice_no'] = $request->request_reference;
        $output['invoice_no_prefix'] = $il->invoice_no_prefix;
        ...
        $output['date_label'] = $il->date_label;
        if (blank($il->date_time_format)) {
            $output['invoice_date'] = $this->format_date($request->created_at, true, $business_details);
        } else {
            $output['invoice_date'] = \Carbon::createFromFormat('Y-m-d H:i:s', $request->created_at)->format($il->date_time_format);
        }

        $output['transaction_date'] = $request->created_at;
        $output['date_time_format'] = $business_details->date_format;
        $output['currency_symbol'] = $business_details->currency_symbol;
        ...
        $lines = $request->items;
            ...
            foreach ($lines as $key => $value) {
                if (!empty($value->product->sub_unit_ids)) {
                    $formated_sell_line = $this->recalculateRequestSellLineTotals($business_details->id, $value);
            ...
            $output['tax_summary_label'] = $il->common_settings['tax_summary_label'] ?? '';
            $details = $this->_receiptRequestDetailsSellLines($lines, $il, $business_details);
            // dd($details);
            $output['lines'] = $details['lines'];
            $output['taxes'] = [];
            $total_quantity = 0;
        ...
        $output['subtotal_label'] = $il->sub_total_label . ':';
        $output['subtotal'] = 0;
        $output['subtotal_unformatted'] = 0;

        // $output['subtotal'] = ($request->total_before_tax != 0) ? $this->num_f($transaction->total_before_tax, $show_currency, $business_details) : 0;
        // $output['subtotal_unformatted'] = ($transaction->total_before_tax != 0) ? $transaction->total_before_tax : 0;

        //round off
        $total_exempt=0;
        $output['round_off_label'] = !empty($il->round_off_label) ? $il->round_off_label . ':' : __('lang_v1.round_off') . ':';
        $output['round_off'] = $this->num_f(0, $show_currency, $business_details);
        $output['round_off_amount'] = 0;
        $output['total_exempt'] = $this->num_f($total_exempt, $show_currency, $business_details);
        $output['total_exempt_uf'] = $total_exempt;
        ...
        //Discount
        $discount_amount = $this->num_f(0, $show_currency, $business_details);
        $output['line_discount_label'] = $invoice_layout->discount_label;
        $output['discount_label'] = $invoice_layout->discount_label;
        $output['discount_label'] .= '<small>(' .  $this->num_f(0, false, $business_details) . '%)</small>';

        $output['discount'] = 0;
        $output['foreign_discount'] =  0;
        ...
        $output['tax_label'] = '';
        $output['tax'] = 0;
        $output['foreign_tax'] = 0;

        $output['group_tax_details'] = [];

        //Shipping charges
        $output['shipping_charges'] =   0;
        $output['foreign_shipping_charges'] = 0;
        $output['shipping_charges_label'] = trans("sale.shipping_charges");
        //Shipping details
        ...
        $output['shipping_details_label'] = trans("sale.shipping_details");
        $output['packing_charge_label'] = trans("lang_v1.packing_charge");
        $output['packing_charge'] = 0;
        $output['foreign_packing_charge'] =  0;

        //Total
        if ($transaction_type == 'sell_return') {
            $output['total_label'] = $invoice_layout->cn_amount_label . ':';
            $output['total'] = $this->num_f(0, $show_currency, $business_details);
        } else {
            $output['total_label'] = $invoice_layout->total_label . ':';
            $output['total'] = $this->num_f(0, $show_currency, $business_details);
        }
        $output['foreign_total'] = 0;

        if (!empty($il->common_settings['show_total_in_words'])) {
            $word_format = isset($il->common_settings['num_to_word_format']) ? $il->common_settings['num_to_word_format'] : 'international';
            $totalForWord = str_replace(",", "", number_format(0, 2));
            $output['total_in_words'] = $this->numToWord($totalForWord , null, $word_format);
            $output['word_format']=$word_format;
        }

        $output['total_unformatted'] = 0;
        
        $output['additional_expenses'] = [];

        //Check for barcode
        $output['barcode'] = ($il->show_barcode == 1) ? $request->request_reference : false;

        //Additional notes
        $output['additional_notes'] = $request->items->pluck('item_notes') ;
        $output['footer_text'] = $invoice_layout->footer_text;
        ...
        // if (in_array($transaction_type, ['sell', 'sales_order'])) {
            ...
            if($zatca_qr){
                $total_order_tax = 0 + $total_line_taxes;
                $qr_code_text = $this->_zatca_qr_text($business_details->name, $business_details->tax_number_1, $request->created_at, 0, 0);
            } else {
                $is_label_enabled = !empty($il->common_settings['show_qr_code_label']) ? true : false;
                $qr_code_details = [];
                $qr_code_fields = !empty($il->qr_code_fields) ? $il->qr_code_fields : [];
                ...
                if (in_array('invoice_no', $qr_code_fields)) {
                    $qr_code_details[] = $is_label_enabled ? $il->invoice_no_prefix . ': ' . $request->request_reference  : $request->request_reference ;
                }
                if (in_array('invoice_datetime', $qr_code_fields)) {
                    $qr_code_details[] = $is_label_enabled ? $output['date_label'] . ': ' . $output['invoice_date'] : $output['invoice_date'];
                }
    ...
    protected function _receiptDetailsSellLines($lines, $il, $business_details)
    {
        ...
        foreach ($lines as $line) {
            ...
            if ($line->line_discount_type == 'percentage') {
                $line_array['line_discount'] = $this->num_f(str_replace(",","",$line_array['line_discount']));
                $line_array['line_discount'] .= ' (' . $this->num_f(str_replace(",","",$line->line_discount_amount), false, $business_details) . '%)';

                $line_array['line_discount_percent'] = $this->num_f(str_replace(",","",$line->line_discount_amount), false, $business_details);
            }
    ...
    protected function _receiptRequestDetailsSellLines($lines, $il, $business_details)
    {
        $is_lot_number_enabled = $business_details->enable_lot_number;
        $is_product_expiry_enabled = $business_details->enable_product_expiry;
        $output_lines = [];
        ...
        foreach ($lines as $line) {
            ...
            $product_variation = $line->variation;
            $unit = $line->product->unit;
            $brand = $line->product->brand;
            $cat = $line->product->category;
            $taxValue=$line->tax? $line->tax.'.0000': '0.0000';
            $tax_details = TaxRate::where('amount',$taxValue)->first();
            ...
            $show_product_description = $il->common_settings['show_product_description'] ?? null;
            $unit_price_inc_tax=$line->sell_price_wot?? $line->variation->default_sell_price;
            $unit_price_inc_tax=(float)$unit_price_inc_tax;
            $unit_price_inc_tax= (float)$unit_price_inc_tax + $line->tax;
            $line_array = [
                //Field for 1st column
                'name' => $product->name,
                'product_description' => !empty($show_product_description) ? $product->product_description : null,
                ...
                'base_unit_name' => $base_unit_name,
                'base_unit_multiplier' => !empty($line->multiplier) && $line->multiplier != 1 ? $this->num_f($line->multiplier, false, $business_details) : 1,
                'orig_quantity' => $this->num_f($line->quantity, false, $business_details, true),
                'unit_price' => $this->num_f((float)$line->sell_price_wot?? $line->variation->default_sell_price , false, $business_details),
                'unit_price_uf' => (float)$line->sell_price_wot?? $line->variation->default_sell_price,
                'tax' => $this->num_f($line->tax, false, $business_details),
                'tax_id' => $line->tax_id,
                'tax_unformatted' => $line->tax,
                'tax_name' => !empty($tax_details) ? $tax_details->name : null,
                'tax_percent' => !empty($tax_details) ? $tax_details->amount : null,

                //Field for 3rd column
                'unit_price_inc_tax' => $this->num_f($unit_price_inc_tax, false, $business_details),
                'unit_price_inc_tax_uf' => $line->unit_price_inc_tax,
                'unit_price_exc_tax' => $this->num_f((float)$line->sell_price_wot?? $line->variation->default_sell_price, false, $business_details),
                'base_unit_price' => $this->num_f((float)$line->sell_price_wot?? $line->variation->default_sell_price, false, $business_details),
                'price_exc_tax' => $line->quantity * (float)$line->sell_price_wot?? $line->variation->default_sell_price,
                'unit_price_before_discount' => $this->num_f((float)$line->sell_price_wot?? $line->variation->default_sell_price, false, $business_details),
                'unit_price_before_discount_uf' => (float)$line->sell_price_wot?? $line->variation->default_sell_price,
                //Fields for 4th column
                'line_total' => $this->num_f($unit_price_inc_tax * $line->quantity, false, $business_details),
                'line_total_uf' => $unit_price_inc_tax * $line->quantity,
                'line_total_exc_tax' => $this->num_f((float)$line->sell_price_wot?? $line->variation->default_sell_price * $line->quantity, false, $business_details),
                'line_total_exc_tax_uf' => (float)$line->sell_price_wot?? $line->variation->default_sell_price * $line->quantity,
                'variation_id' => $variation->id
            ];
            ...
            if (!empty($tax_details)) {
                if ($tax_details->is_tax_group) {
                    $group_tax_details = $this->groupTaxDetails($tax_details, $line->quantity * $line->tax);
                    ...
                    $output_taxes['taxes'][$tax_name] += ($line->quantity * $line->tax);
                }
            }

            $line_array['line_discount'] = $this->num_f($this->get_request_discount_amount($line), false, $business_details);
            $line_array['line_discount_uf'] = $this->get_request_discount_amount($line);

            if ($line->line_discount_type == 'percentage') {
                $line_array['line_discount'] = $this->num_f(str_replace(",","",$line_array['line_discount']));
                $line_array['line_discount'] .= ' (' . $this->num_f(str_replace(",","",$line->discount), false, $business_details) . '%)';

                $line_array['line_discount_percent'] = $this->num_f(str_replace(",","",$line->discount), false, $business_details);
            }
    ...
    public function get_request_discount_amount($line)
    {
        $discount_amount = 0;
        if (!empty($line->discount_type) && !empty($line->discount)) {
            if ($line->discount_type == 'fixed') {
                $discount_amount = $line->discount;
            } elseif ($line->discount_type == 'percentage') {
                $discount_amount = ((float)$line->sell_price_wot?? $line->variation->default_sell_price * $line->discount) / 100;
            }
        }
    ...
    */
    ...
    public function getCurrencyByTransactionId($transaction_id) {
        $transaction = Transaction::find($transaction_id);
        $location = BusinessLocation::find($transaction->location_id);
        $currency = Currency::where('id', $location->currency_id)->first();
        return $currency;
    }
    ...
    {
        $total_paid = (int)str_replace(",","", number_format($this->getTotalPaid($transaction_id), 2)) * 1000;
        $transaction = Transaction::find($transaction_id);
        if (is_null($final_amount)) {
            ...
            if($transaction->type != 'expense') {
                $location = BusinessLocation::find($transaction->location_id);
                $currency = Currency::where('id', $location->currency_id)->first();
                $final_amount = str_replace(",","", number_format($transaction->final_total , 2));
            }
        }
        if($transaction->type == 'expense') {
            $location = BusinessLocation::find($transaction->location_id);
            $currency = Currency::where('id', $location->currency_id)->first();
            $final_amount = (float)str_replace(",", "", number_format($transaction->final_total , 2));
        }
        $final_amount = (int)$final_amount * 1000;
        $status = 'due';
     ...
     * @return object
     */
    public function purchaseCurrencyDetails($business_id, $locationId=0)
    {
        ...
        if($location!=null){
            $currency = Currency::find($location->currency_id);
     ...
     * @return object
     */
    public function purchaseCurrencyDetailsByLocation($location_id, $business_id)
    {
        $business = Business::find($business_id);
        ...
        if ($business->purchase_in_diff_currency == 1) {
            ...
            $currency_id = $business_location->currency_id;
        } else {
            $output['purchase_in_diff_currency'] = false;
            $output['p_exchange_rate'] = 1;
            $currency_id = $business_location->currency_id;
    ...
    public function mapPurchaseSell($business, $transaction_lines, $mapping_type = 'purchase', $check_expiry = true, $purchase_line_id = null)
    {
        ...
        foreach ($transaction_lines as $line) {
            ...
            foreach ($rows as $k => $row) {
                ...
                $row->quantity_available=$row->quantity_available+$row->quantity_sold; // Added by Bilal
    ...
    */
    ...
    public function recalculateRequestSellLineTotals($business_id, $sell_line)
    {
        $unit_details = $this->getSubUnits($business_id, $sell_line->product->unit->id);

        $sub_unit = null;
        $sub_unit_id = $sell_line->product->sub_unit_ids;
        foreach ($unit_details as $key => $value) {
            if ($key == $sub_unit_id) {
                $sub_unit = $value;
            }
        ...
        if (!empty($sub_unit)) {
            ...
            if ($sell_line->discount_type == 'fixed') {
                $sell_line->discount = $sell_line->discount * $multiplier;
            }
            $sell_line->quantity = $sell_line->quantity;
            $sell_line->multiplier = $multiplier;
            $sell_line->quantity = $sell_line->quantity / $multiplier;
            $sell_line->unit_price_before_discount = $sell_line->sell_price_wot?? $sell_line->variation->default_sell_price * $multiplier;
            $sell_line->unit_price = $sell_line->sell_price_wot?? $sell_line->variation->default_sell_price * $multiplier;
            $unitPriceWTax=$sell_line->sell_price_wot?? $sell_line->variation->default_sell_price + $sell_line->discount?? 0;
            $sell_line->unit_price_inc_tax = $unitPriceWTax * $multiplier;
            $sell_line->item_tax = $sell_line->tax * $multiplier;
    ...
    */
    ...
    public function createExpense($request, $business_id, $user_id, $format_data = true)
    {
        ...
        $this->updatePaymentStatus($transaction->id, str_replace(",","",$transaction->final_total) );
