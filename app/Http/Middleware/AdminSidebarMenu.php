        ...
        Menu::create('admin-sidebar-menu', function ($menu) {
                ...
                $menu->dropdown(
                    ...
                    function ($sub) use ($enabled_modules,$common_settings) {
                        if (!empty($common_settings['enable_purchase_requisition']) && (auth()->user()->can('purchase_requisition.view_all') || auth()->user()->can('purchase_requisition.view_own')) ) {
                            $sub->url(
                                action('PurchaseController@addRequest'),
                                __('request.request_a_quotes'),
                                ['icon' => 'fa fas fa-list', 'active' => request()->segment(2) == 'add_requests']
                            );
                        }
                        ...
                        if (!empty($common_settings['enable_purchase_order']) && (auth()->user()->can('purchase_order.view_all') || auth()->user()->can('purchase_order.view_own')) ) {
                            $sub->url(
                                action('PurchaseController@listRequests'),
                                __('request.content_request'),
                                ['icon' => 'fa fas fa-list', 'active' => request()->segment(1) == 'requests' && request()->segment(2) != 'ready-to-draft' && request()->segment(2) != 'items' && request()->segment(2) != 'draft' && request()->path() !='requests/list/quote']
                            );
                        }
                        if (!empty($common_settings['enable_purchase_order']) && (auth()->user()->can('purchase_order.view_all') || auth()->user()->can('purchase_order.view_own')) ) {
                            $sub->url(
                                action('PurchaseController@requestitems'),
                                __('request.pending_request'),
                                ['icon' => 'fa fas fa-list', 'active' => request()->segment(2) == 'items']
                            );
                        }
                        if (!empty($common_settings['enable_purchase_order']) && (auth()->user()->can('purchase_order.view_all') || auth()->user()->can('purchase_order.view_own')) ) {
                            $sub->url(
                                action('RequestController@readyToDraftList'),
                                __('request.ready_draft_request'),
                                ['icon' => 'fa fas fa-list', 'active' => request()->segment(2) == 'ready-to-draft']
                            );
                        }
                        if (!empty($common_settings['enable_purchase_order']) && (auth()->user()->can('purchase_order.view_all') || auth()->user()->can('purchase_order.view_own')) ) {
                            $sub->url(
                                action('RequestController@draftList'),
                                __('request.draft_request'),
                                ['icon' => 'fa fas fa-list', 'active' => request()->segment(2) == 'draft']
                            );
                        }
                        if (!empty($common_settings['enable_purchase_order']) && (auth()->user()->can('purchase_order.view_all') || auth()->user()->can('purchase_order.view_own')) ) {
                            $sub->url(
                                action('RequestController@listQuote'),
                                __('request.list_quote_request'),
                                ['icon' => 'fa fas fa-list', 'active' => request()->path()=='requests/list/quote']
                            );
                        }
                        if (!empty($common_settings['enable_purchase_order']) && (auth()->user()->can('purchase_order.view_all') || auth()->user()->can('purchase_order.view_own')) ) {
                            $sub->url(
                                action('RequestController@acceptedQuote'),
                                __('request.accepted_quote_request'),
                                ['icon' => 'fa fas fa-list', 'active' => request()->path()=='request/quote/accepted']
                            );
                        }
                        if (!empty($common_settings['enable_purchase_order']) && (auth()->user()->can('purchase_order.view_all') || auth()->user()->can('purchase_order.view_own')) ) {
                            $sub->url(
                                action('RequestController@listRejected'),
                                __('request.rejected_quote_request'),
                                ['icon' => 'fa fas fa-list', 'active' => request()->path()=='request/quote/rejected']
                            );
                        }
                        if (!empty($common_settings['enable_purchase_order']) && (auth()->user()->can('purchase_order.view_all') || auth()->user()->can('purchase_order.view_own')) ) {
                            $sub->url(
                                action('RequestController@listDisputed'),
                                __('request.dispute_quote_request'),
                                ['icon' => 'fa fas fa-list', 'active' =>request()->path()=='request/quote/disputed']
                            );
                        }
                        if (!empty($common_settings['enable_purchase_order']) && (auth()->user()->can('purchase_order.view_all') || auth()->user()->can('purchase_order.view_own')) ) {
                            $sub->url(
                                action('PurchaseController@listRequests',['status'=>'pendingPurchaseRequest']),
                                __('request.pending_purchase_request'),
                                ['icon' => 'fa fas fa-list', 'active' => request()->query('status') == 'pendingPurchaseRequest']
                            );
                        }
                        if (!empty($common_settings['enable_purchase_order']) && (auth()->user()->can('purchase_order.view_all') || auth()->user()->can('purchase_order.view_own')) ) {
                            $sub->url(
                                action('PurchaseController@listRequests',['status'=>'purchaseOrderListRequest']),
                                __('request.purchase_order_list_request'),
                                ['icon' => 'fa fas fa-list', 'active' => request()->query('status') == 'purchaseOrderListRequest']
                            );
                        }
                    },
            ...
            if (in_array('purchases', $enabled_modules) && (auth()->user()->can('purchase.view') || auth()->user()->can('purchase.create') || auth()->user()->can('purchase.update'))) {
                $menu->dropdown(
                    ...
                    function ($sub) use ($enabled_modules,$is_admin,$common_settings) {
                        if (!empty($common_settings['enable_purchase_requisition']) && (auth()->user()->can('purchase_requisition.view_all') || auth()->user()->can('purchase_requisition.view_own')) ) {
                            $sub->url(
                        ...
                        if (in_array('add_sale', $enabled_modules) && ( $is_admin || auth()->user()->hasAnyPermission(['quotation.view_all', 'quotation.view_own'])) ) {
                            $sub->url(
                                action('PurchaseController@addRequest'),
                                __('request.add_request'),
                                ['icon' => 'fa fas fa-pen-square', 'active' => request()->segment(1) == 'sells' && request()->segment(2) == 'add_requests']
                            );
                        }
                    },
                    ['icon' => 'fa fas fa-arrow-circle-down', 'id' => 'tour_step6']
            ...
            if ($is_admin || auth()->user()->hasAnyPermission(['sell.view', 'sell.create', 'direct_sell.access', 'view_own_sell_only', 'view_commission_agent_sell', 'access_shipping', 'access_own_shipping', 'access_commission_agent_shipping', 'access_sell_return', 'direct_sell.view', 'direct_sell.update', 'access_own_sell_return']) ) {
                $menu->dropdown(
                    ...
                    function ($sub) use ($enabled_modules, $is_admin, $pos_settings) {
                        ...
                        // if (in_array('add_sale', $enabled_modules) && ( $is_admin || auth()->user()->hasAnyPermission(['quotation.view_all', 'quotation.view_own'])) ) {
                        //     $sub->url(
                        //         action('SellPosController@listRequess'),
                        //         __('lang_v1.list_requests'),
                        //         ['icon' => 'fa fas fa-pen-square', 'active' => request()->segment(1) == 'sells' && request()->segment(2) == 'list_requests']
                        //     );
                        // }
            ...
            if ((auth()->user()->can('currency.view') || auth()->user()->can('currency.create'))) {
                $menu->dropdown(
                    __('lang_v1.list_currency'),
                    function ($sub) {
                        if (auth()->user()->can('currency.view')) {
                            $sub->url(
                                action('CurrencyController@index'),
                                __('lang_v1.list_currency'),
                                ['icon' => 'fa fas fa-list', 'active' => request()->segment(1) == 'currency' && request()->segment(2) == null]
                            );
                        }
                    },
