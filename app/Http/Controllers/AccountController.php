...
class AccountController extends Controller
{
    ...
    public function index()
    {
        ...
        if (request()->ajax()) {
            ...
            ->leftJoin('users AS u', 'accounts.created_by', '=', 'u.id')
                                ...
                                ->select(['accounts.name','accounts.currency_id', 'accounts.account_number', 'accounts.note', 'accounts.id', 'accounts.account_type_id',
            ...
            return DataTables::of($accounts)
                            ...
                            ->editColumn('balance', function ($row) {
                                if($row->currency_id) {
                                    ...
                                    return '<span class="balance" data-orig-value="' . $row->balance . '">' . $currency->symbol . number_format($row->balance , 4) . '</span>';
                                }
    ...
    public function create()
    {
        ...
        return view('account.create')
                ->with(compact('account_types', 'currencies'));
    }
    ...
    public function store(Request $request)
    {
        ...
        if (request()->ajax()) {
            try {
                $input = $request->only(['name', 'account_number', 'note', 'account_type_id', 'account_details', 'currency_id']);
            ...
            return DataTables::of($accounts)
                        ...
                        ->addColumn('payment_details', function($row){
                            ...
                            ->addColumn('debit', function ($row) {
                                $currency = Currency::find($row->account_currency_id);
                                if ($row->type == 'debit') {
                                    if($row->transaction_id) {
                                        $location_id = Transaction::where('id', $row->transaction_id)->pluck('location_id');
                                        $location = BusinessLocation::where('id', $location_id[0])->first();
                                        $currency_rate = Currency::find($location->currency_id)->rate;
                                        return '<span class="credit" data-orig-value="' . $row->amount . '">' . $currency->symbol .''. number_format($row->amount , 4) . '</span>';
                                    }
                                    return '<span class="credit" type="'.$row->type.'"  data-orig-value="' . $row->amount . '">' . $currency->symbol .''. number_format($row->amount, 4) . '</span>';
                                }
                                return '';
                            })
                            ->addColumn('credit', function ($row) {
                                ...
                                if ($row->type == 'credit') {
                                    if($row->transaction_id) {
                                        $transaction = Transaction::where('id', $row->transaction_id)->first();
                                        $location = BusinessLocation::where('id', $transaction->location_id)->first();
                                        $currency_rate = Currency::find($location->currency_id)->rate;
                                        $currency_rate = $transaction->type == 'purchase' ? $currency_rate : 1;
                                        // return '<span class="debit" data-orig-value="' . $row->amount * $currency_rate . '" symbol="'.$currency->symbol.'">' . $currency->symbol .''. number_format($row->amount * $currency_rate, 4) . '</span>';
                                        return '<span class="debit" data-orig-value="' . $row->amount . '" symbol="'.$currency->symbol.'">' . $currency->symbol .''. number_format($row->amount, 4) . '</span>';
                                    }
                                    return '<span class="debit" data-orig-value="' . $row->amount . '">' . $currency->symbol .''. number_format($row->amount, 4) . '</span>';
                                }
                                return '';
                            })
                            ->addColumn('balance', function ($row) use ($bal_before_start_date, $start_date) {
                                ...
                                $acc_trans = AccountTransaction::where('account_id', 
                                                    $row->account_id)
                                                ->where('operation_date', '>=', $start_date)
                                                ->where('operation_date', '<=', $row->operation_date)->get();
                                $current_bal = 0;
                                foreach($acc_trans as $i => $acc) {
                                    $currency_rate = 1;
                                    if($acc->transaction_id) {
                                        $transaction = Transaction::where('id', $acc->transaction_id)->first();
                                        if($transaction->location_id && $transaction->type == 'purchase') {
                                            $location = BusinessLocation::where('id', $transaction->location_id)->first();
                                            $currency_rate = Currency::find($location->currency_id)->rate;
                                        }
                                    }
                                    // $current_bal += $acc->type == 'credit' ? $acc->amount * $currency_rate : -1 * $acc->amount * $currency_rate;
                                    $current_bal += $acc->type == 'credit' ? $acc->amount  : -1 * $acc->amount ;
                                }
                                // return
                                // return json_encode($acc_trans);
                                ...
                                return '<span class="balance" current_bal="'.$bal_before_start_date.'" data-orig-value="' . $bal . '">' . $currency->symbol .''. number_format($bal, 4) . '</span>';
                            })
    ...
    public function edit($id)
    {
        ...
        if (request()->ajax()) {
            ...
            return view('account.edit')
                ->with(compact('account', 'account_types', 'currencies'));
        }
    }
