...
class BusinessController extends Controller
{
    ...
    */
    ...
    public function postRegister(Request $request)
    {
        ...
        try {
            $validator = $request->validate(
                ...
                'currency_exchange_rate.required' => __('validation.required', ['attribute' => __('Currency Exchange Rate')]),
                ]
            );
            ...
            $business_details['enabled_modules'] = ['purchases','add_sale','pos_sale','stock_transfers','stock_adjustment','expenses', 'currency_exchange_rate'];
    ...
    public function getBusinessSettings()
    {
        ...
        $default_currency = Currency::where('code', $currency->code)->where('business_id', $business_id)->first();
        ...
        $currency_exchange_rate = empty($business->currency_exchange_rate) ? 1 : $business->currency_exchange_rate;
        ...
        return view('business.settings', compact('business', 'currencies', 'tax_rates', 'timezone_list', 'months', 'accounting_methods', 'commission_agent_dropdown', 'units_dropdown', 'date_formats', 'shortcuts', 'pos_settings', 'modules', 'theme_colors', 'email_settings', 'sms_settings', 'mail_drivers', 'allow_superadmin_email_settings', 'custom_labels', 'common_settings', 'weighing_scale_setting', 'payment_types','currency_exchange_rate'));
    }
    ...
    public function postBusinessSettings(Request $request)
    {
        ...
        try {
            ...
            $business_details = $request->only(['name', 'start_date', 'currency_id', 'tax_label_1', 'tax_number_1', 'tax_label_2', 'tax_number_2', 'default_profit_percent', 'default_sales_tax', 'default_sales_discount', 'sell_price_tax', 'sku_prefix', 'time_zone', 'fy_start_month', 'accounting_method', 'transaction_edit_days', 'sales_cmsn_agnt', 'item_addition_method', 'currency_symbol_placement', 'on_product_expiry',
                ...
                'code_label_1', 'code_1', 'code_label_2', 'code_2', 'currency_precision', 'quantity_precision', 'currency_exchange_rate']);
            ...
            $business_details['currency_exchange_rate'] = !empty($business_details['currency_exchange_rate']) ? $this->businessUtil->num_uf($business_details['currency_exchange_rate']) : 1;
            ...
            $request->session()->put('currency_exchange_rate', $business->currency_exchange_rate);
     ...
     public function reset_database(Request $request)
    {
        ...
        return view('reset_database',compact('business_locations'));
    }
    
    public function reset_database_delete(Request $request)
    {
        // if ($request->location_id){
            $business_id = request()->session()->get('user.business_id');
            $location_id = $request->get('location_id');
            $transactions_ids = Transaction::where('type','sell')
                ->orwhere('type','sell_return')
                ->orwhere('type','sell_transfer')
                ->orwhere('type','purchase')
                ->orwhere('type','purchase_return')
                ->orwhere('type','purchase_transfer')
            ...
            VariationLocationDetails::where('location_id',$location_id)->delete();
            $output = [
                'success' => 1,
