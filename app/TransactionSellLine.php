...
class TransactionSellLine extends Model
{
    ...
    public function get_request_discount_amount()
    {
        $discount_amount = 0;
        if (!empty($this->discount_type) && !empty($this->discount)) {
            if ($this->discount_type == 'fixed') {
                $discount_amount = $this->discount_amount;
            } elseif ($this->discount_type == 'percentage') {
                $discount_amount = ($this->unit_price_before_discount * $this->discount) / 100;
            }
        }
        return $discount_amount;
    }
