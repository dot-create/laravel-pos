...
class AddFieldsToInvoiceSchemesTable extends Migration
{
    ...
    public function up()
    {
        Schema::table('invoice_schemes', function (Blueprint $table) {
            ...
            $table->integer('current_number')->nullable()->after('end_number');
            $table->date('start_date')->nullable()->after('current_number');
    ...
    {
        Schema::table('invoice_schemes', function (Blueprint $table) {
            $table->dropColumn(['status', 'end_number', 'start_date', 'expiration_date', 'invoicing_key', 'current_number']);
        });
    }
}
